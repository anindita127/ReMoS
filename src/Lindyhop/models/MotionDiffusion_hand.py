...
class DiffusionTransformer(nn.Module):
    def __init__(self,
        ...
        self.input_condn_feats = input_condn_feats
        self.input_feats = input_feats
        self.time_embed_dim = latent_dim 
        self.spatio_temp = self.num_frames * 2 * self.num_jts
    ...
    def forward(self, motion2, timesteps, motion1=None, spatial_guidance=None):
        """
        x: B, T, D
        """
        B, T, D = motion1.shape
        contact_distance = motion1[:, :, -4:]
        rh_rh = contact_distance[:,:, 0] == 1
        rh_lh = contact_distance[:,:, 1] == 1
        lh_rh = contact_distance[:,:, 2] == 1
        lh_lh = contact_distance[:,:, 3] == 1
        rh_pose1 = torch.logical_or(rh_lh, rh_rh)
        lh_pose1 = torch.logical_or(lh_lh, lh_rh)
        m1 = self.motion1_pre_proj(motion1[:, :, :-4].reshape(B, T, -1, 3))    # GCN
        m2 = self.motion2_pre_proj(motion2.reshape(B, T, -1, 3))    # GCN
        m1 = m1.reshape(B, T*2*self.num_jts, -1)
        m2 = m2.reshape(B, T*2*self.num_jts, -1)
        src_mask = torch.zeros(B, T, self.num_jts*2).to(m1.device).float()
        src_mask[:, :, 0] = rh_pose1
        src_mask[:, :, 1] = rh_pose1
        src_mask[:, :, 2] = rh_pose1
        src_mask[:, :, 3] = rh_pose1
        ...
        src_mask[:, :, 8] = rh_pose1
        src_mask[:, :, 9] = rh_pose1
        src_mask[:, :, 10] = rh_pose1
        src_mask[:, :, 11] = lh_pose1
        src_mask[:, :, 12] = lh_pose1
        src_mask[:, :, 13] = lh_pose1
        src_mask[:, :, 14] = lh_pose1
        src_mask[:, :, 15] = lh_pose1
        src_mask[:, :, 16] = lh_pose1
        src_mask[:, :, 17] = lh_pose1
        src_mask[:, :, 18] = lh_pose1
        src_mask[:, :, 19] = lh_pose1
        src_mask[:, :, 20] = lh_pose1
        src_mask[:, :, 21] = lh_pose1
        src_mask = src_mask.reshape(B, -1)
        src_mask =  torch.repeat_interleave(src_mask.unsqueeze(-1), src_mask.shape[1], axis=-1)
        src_mask =  torch.repeat_interleave(src_mask.unsqueeze(-1), self.num_heads, axis=-1)
        ...
        output = self.out(h_pe).view(B, T, -1).contiguous()
        # if timesteps == 400:
